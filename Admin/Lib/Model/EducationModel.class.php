...
class EducationModel extends Model {
	...
	function attr_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
	{
		...
		if ($res === NULL)
		{
			...
			// ", COUNT(a.article_id) AS aricle_num ".
			   " FROM " . C('DB_PREFIX') . "educationattr_list AS c".
			   " LEFT JOIN " . C('DB_PREFIX') . "educationattr_list AS s ON s.parent_id=c.attr_id".
			   // " LEFT JOIN " . C('DB_PREFIX') . "article AS a ON a.cat_id=c.cat_id".
			   " GROUP BY c.attr_id ".
			   " ORDER BY parent_id, sort_order ASC";
			$model = M("educationattr_list");
	...
	{
		$M_attr_list=M('educationattr_list');
		if ($cat_id > 0)
		{
		...
		*/
		...
		if ($sameGradeCount || $parent_id == 0)
		{
			...
			$sql = 'SELECT attr_id AS cat_id,attr_name ,parent_id,sort_order ' .
					'FROM ' . C('DB_PREFIX') . 'educationattr_list ' .
	...
	function cat_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
	{
		...
		if ($res === NULL)
		{
			...
			// ", COUNT(a.article_id) AS aricle_num ".
			   " FROM " . C('DB_PREFIX') . "educationcat_list AS c".
			   " LEFT JOIN " . C('DB_PREFIX') . "educationcat_list AS s ON s.parent_id=c.cat_id".
			   // " LEFT JOIN " . C('DB_PREFIX') . "article AS a ON a.cat_id=c.cat_id".
			   " GROUP BY c.cat_id ".
			   " ORDER BY parent_id, sort_order ASC";
			$model = M("educationcat_list");
	...
	{
		$M_cat_list=M('educationcat_list');
		if ($cat_id > 0)
		{
		...
		*/
		...
		if ($sameGradeCount || $parent_id == 0)
		{
			...
			$sql = 'SELECT cat_id AS cat_id,cat_name ,parent_id,sort_order ' .
					'FROM ' . C('DB_PREFIX') . 'educationcat_list ' .
