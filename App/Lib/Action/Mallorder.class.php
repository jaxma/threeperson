...
//        $price_key_name = "price" . $manager['level'];//该用户相应等级的产品单价key名
        ...
        foreach( $p_ids as $p_key => $p_id ){
            ...
            if(C('ORDER_SHIPPING')){
                $check_cart_ids=array_filter($cart_ids);

                if (empty($check_cart_ids)) {
                    $condition=[
                        'id'=>$shipping_way_id,
                    ];
                    //产品参数
                    ...
                    $Order = new Order();
                    $shipping_fee=$Order->get_shipping_fee($p_num,$the_total_price,$condition,$product_parameter,$template_id);
