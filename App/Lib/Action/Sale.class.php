...
//        }
        ...
        $old_info = [];
        if( !empty($id) ){
            $old_info = $sale_lunpan_set_obj->where($condition)->order('id asc')->find();
        }
        
        
        
        if( !empty($old_info) ){
            if( $new_info['name'] == $old_info['name'] && $new_info['percent'] == $old_info['percent'] && $new_info['total_num'] == $old_info['total_num'] && $new_info['img'] == $old_info['img']){
                $return_result = [
                    'code'  =>  3,
                    'msg'   =>  '没有做出改动！',
                ];
