...
//		var_dump($this->templet['properties']);die;
...
//		$this->trigger(self::EVENT_AFTER_SAVE_PROPERTY_VALUES);
  ...
  // 若更改该方法的$key格式，请把下面的linearName方法也作相应修改
...
//                break;
    ...
    public function recursion($properties,$name,$id){
      $this->linearName($properties,'');
      foreach($this->templet_name_arr as $k => $v){
        $property = explode(";",$v);
        $property_name = $name.'---';
        foreach($property as $ko => $vo){
          $voo = explode(":",$vo);
          $property_name .= array_pop($voo);
          if($property[$ko+1]!=NULL){
            $property_name .= ' x ';
          }
        }
//      var_dump($property_name);die;
        $result[$k]['id'] = $id;
        $result[$k]['name'] = $property_name;
        $result[$k]['properties'] = $v;
      }
      
      return $result;
    }
    
    private function linearName($properties, $nameKey = '') {
      if (empty($properties)) {
        return;
      }
      $property = array_shift($properties);
      ...
      $id = $property['pid'];
      foreach ($property['values'] as $k => $v) {
        $key = $nameKey."$id:$v[vid]:$v[value]";
        if (!empty($properties)) {
          $key .= $this->linearName($properties, $key);
          continue;
        }
        
        $this->templet_name_arr[] = $key;
//      if (isset($this->oldPropertyPrices[$key])) {
//        $this->templet['propertyPrices'][$key] = $this->oldPropertyPrices[$key];
//      } else {
//        $this->templet['propertyPrices'][$key] = ['price'=>0, 'stock'=>1];
