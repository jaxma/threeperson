...
class SearchAction extends CommonAction {
    ...
    public function index() {
    	...
    	if($this->lang){
    		$plist = $Model->where("isopen=1 and (  title_en like '%%d%' or content_en like '%%f%')",array($title_en,$content_en))->select();
    	}else{
    		$plist = $Model->where("isopen=1 and ( title like '%%d%' or content like '%%f%')",array($title,$content))->select();
    	}
