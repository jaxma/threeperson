...
class OrderAction extends CommonAction {
    ...
    public function index() {
        ...
        $get_s_name = trim(I('get.s_name'));//收货人
...
//                    's_id' => 0, //供货商
    ...
    //                        's_id' => $lista['id'], //供货商
        ...
        $condition['status'] = array('not in',[15]);
        if (!empty($get_status)) {
        ...
        if ($count > 0) {
            ...
            //订单信息
            if ($get_status == 1 || $get_status == "") {
                $sort = 'time desc';
            } else {
                $sort = 'paytime desc';
            }
            $applyList = $order->where($condition)->order($sort)->group('order_num')->limit($limit)->select();
    ...
    public function applyList() {
        ...
        if ($count > 0) {
            ...
            foreach ($applyList as $k => $v) {
                ...
                if(!empty($agent_name)&&$dis_info[$v_uid]['name']!=$agent_name){
                    unset($applyList[$k]);
                    continue;
                }
...
//        }
    ...
    public function audit() {
        ...
        $Order = new Order();
        if($pass == 1){
            $order_audit_result = $Order->junior_audit_order($order_nums);
            if( $order_audit_result['code'] == 1 ){
                $this->add_active_log('订单初步审核：'.$order_audit_result['msg']);
            }
        }
        elseif($pass == 8){
            ...
            if( $order_audit_result['code'] == 1 ){
                $this->add_active_log('订单财务审核：'.$order_audit_result['msg']);
            }
        }//完成发货
        elseif ($pass == 6){
            $order_num=[];
...
//        return;
    ...
    public function set_order(){
        ...
        foreach ($templet_info as $k => $v) {
            ...
            $templet_count++;
            //属性相关
            $templet_info[$k]['properties']=$this->get_properties($v['id']);
        }
...
//        print_r($this->_post());return;
    ...
    public function recehand(){
        ...
        $city=trim(I('city'));
        $area=trim(I('area'));
        $address=trim(I('addre'));
        $address_model=M('address');
        if(empty($name) || empty($phone) || empty($province) || empty($city) || empty($area) || empty($address) || empty($address_model)){
            $return_result=[
                'code'=>'-2',
                'msg'=>'收货信息不完整'
            ];
        ...
        //判断这个人有没有地址
        $address_count=$address_model->where(['user_id'=>$id])->count('id');
        if($address_count>0){
            //将默认地址改为非默认地址
            $address_model->where(['user_id'=>$id])->save(['default'=>0]);
    ...
    public function send_order(){
        ...
        $shipper = AllShipperCode();
            ...
            //look 后台审核最高级别代理的订单
            $applyList = $order->order('paytime desc')->where($condition)->group('order_num')->select();
...
//      $this->display();
    ...
    //------------------*****iframe弹出事件****-------------
    public function order_iframe(){
        $id=trim(I('id'));
        $info=M('order')->find($id);
        $this->pay_photo=$info['pay_photo'];
        $this->display();
    }
}
