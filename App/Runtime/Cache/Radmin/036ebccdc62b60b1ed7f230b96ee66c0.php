            ...
            if(data.info.one!=null){
                $.each(data.info.one, function(k, v) {
                  ...
                  html += '<tr data-id=' + v.id + ' data-level=1 >' +
                      ...
                      '<td align="left"><span class="glyphicon glyphicon-plus" data-key="'+v.id+'" style="margin:0 5px;padding:6px 10px;cursor: pointer; background: #337ab7;border-radius: 5px;color: #fff"></span>' + 
                      v.name +'<input type="button" class="btn btn-info add pull-right" value="添加子分类"  data-title="添加分类" data-modal="__URL__/category_add?p_id='+v.id+'&c_id=0" />'+'</td>' +
                      '<td>' + v.sequence + '</td>' +
                    ...
                    if(data.info.two != null) {
                        $.each(data.info.two, function(k, v) {
                            ...
                            html += '<tr data-id=' + v.id + ' data-num=g' + id +
                                ...
                                v.name + '<input type="button" class="btn btn-info add pull-right" value="添加子分类"  data-title="添加分类" data-modal="__URL__/category_add?p_id='+id+'&c_id='+v.id+'"/>'+'</td>' +
                                '<td>' + v.sequence + '</td>' +
                                '<td data-status=' + v.status + '>' + str + '</td>' +
                                '<td>' + d.Format("yyyy-MM-dd hh:mm:ss") + '</td>' +
                                '+<td>' +
...
//      window.location.href = '__URL__/category_edit?id=' + id;
//  })
...
//      var p_id = 0;
//      if($(this).parent().parent().data('num')){
//          p_id = Number($(this).parent().parent().data('num').substr(1))
//      }
//      var c_id = $(this).parent().parent().data('id');
//      if($(this).parent().parent().data('level')=='1'){
//          p_id = c_id;
//          c_id = 0;
//      }
//      window.location.href = '__URL__/category_add?p_id=' + p_id + '&c_id=' + c_id
//  });
