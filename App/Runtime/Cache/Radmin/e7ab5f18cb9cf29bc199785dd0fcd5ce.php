...
<div class="wrapper">
  ...
  <button type="button" class="layui-btn orange layui-btn-danger upload-btn"><i class="layui-icon">&#xe67c;</i>上传图片</button>
  <div class="layui-upload layui-inline">
    <div class="layui-upload-list" data-show="1" data-url="__ROOT__<?php echo ($row["image1"]); ?>">
      <img class="layui-upload-img">
      <p class="demoText"><i class="layui-icon delete" style="font-size: 26px;color: white;line-height: 27px;">&#xe640;</i></p>
    </div>
    <input type="hidden" class="image-name" name="image1" value="<?php echo ($row["image1"]); ?>" />
  </div>
</div>
						<?php else: ?>
...
<div class="wrapper">
  ...
  <button type="button" class="layui-btn orange layui-btn-danger upload-btn"><i class="layui-icon">&#xe67c;</i>上传图片</button>
  <div class="layui-upload layui-inline">
    <div class="layui-upload-list" data-show="0" data-url="__ROOT__<?php echo ($row["image1"]); ?>">
      <img class="layui-upload-img">
      <p class="demoText"><i class="layui-icon delete" style="font-size: 26px;color: white;line-height: 27px;">&#xe640;</i></p>
    </div>
    <input type="hidden" class="image-name" name="image1" value="<?php echo ($row["image1"]); ?>" />
        ...
        $('.ueditors').each(function(key,value){
          ...
          var ue = UE.getEditor($(value).attr("id"),{initialFrameWidth:'100%',initialFrameHeight:350});
			...
			<div class="layui-form-item items">
				<label class="form-text">详情：</label>
				<div class="form-right">
					<input type="button" class="btn btn-success" data-modal="__URL__/img?pid=<?php echo ($row["id"]); ?>" value="上传多图"/>
					<small class="orange-text desc">9张一组，第一张为大图</small>
				  	<?php if(is_array($imgs)): foreach($imgs as $key=>$i): if($i["many_image"] != ''): ?><div class="">
						  <blockquote class="layui-elem-quote" style="margin-top: 10px;">
							<input type="button" class="btn btn-success" data-modal="__URL__/img?id=<?php echo ($i["id"]); ?>&pid=<?php echo ($row["id"]); ?>" value="编辑"/>
                            <input type="button" class="btn btn-danger" value="删除"  data-load='__URL__/del_bi?id=<?php echo ($i["id"]); ?>&pid=<?php echo ($row["id"]); ?>' data-confirm="您确定要删除该图片组吗？"/>
                            <span class="form-text-span">优先级：<?php echo ($i["sequence"]); ?> </span>
                            <span class="form-text-span"><?php if($i["status"] == 1): ?>开启<?php else: ?>关闭<?php endif; ?> </span>
						    <ul class="layui-upload-others">
					  			<?php if(is_array($i["images"])): foreach($i["images"] as $key=>$ii): ?><li class="img-item">
							    		<img class="layui-upload-img" src="__APP__<?php echo ($ii); ?>"/>
