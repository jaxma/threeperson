...
function delGoods(id) {
	$.dialog.confirm('你确定要删除这个课程吗？', function(){
...
.clcl:hover{color:#ff0000;}
    ...
    <div class="column_Box mainAutoHeight">
        <div class="tab">
            <ul>
                <li class="current"><a href="javascript:">课程列表</a></li>
            </ul>
        </div>
        <div class="wrapBox mainAutoHeight">
            ...
            <div class="body User">
                <div class="item">
                    <a href="javascript:void(0);" class="dot_Item"><span class="Icon_item icon_xingjian"></span><i><input type="button" value="添加课程" class="submitNoBg" onclick="window.location.href='<?php echo U('Goods/add',array('cat_id'=>$_GET['cat_id']));?>'"/></i></a>
                    <div class="searchBar">
						<form action="<?php echo U('Goods/index');?>">
							...
							<select name="sort_by" class="dot_Item">
								...
								<option value="a.title" <?php if($filter["sort_by"] == 'a.title'): ?>selected=""<?php endif; ?>>课程标题</option>
								<option value="ac.cat_name" <?php if($filter["sort_by"] == 'ac.cat_name'): ?>selected=""<?php endif; ?>>课程分类</option>
	...
	/**
	  ...
	  function confirmSubmit(frm, ext){
		  if (frm.elements['type'].value == 'button_remove'){
			  return confirm('您确定要删除课程吗');
		  }else if (frm.elements['type'].value == 'not_on_sale'){
			  return confirm('您确定要隐藏课程吗');
