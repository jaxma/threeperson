...
//  var input = document.getElementById("myfile1");
    ...
    if(typeof FileReader === 'undefined'){
        $('.img-list').empty().append('<p class="warn">抱歉，你的浏览器不支持 FileReader</p>');
        $('.btn-upload').attr('disabled',true);
//      result.innerHTML = "<p class='warn'>抱歉，你的浏览器不支持 FileReader</p>";
//      input.setAttribute('disabled','disabled');
    }else{
      $(document).on('change','.btn-upload',function(){
        var count = $('.img-list img').length+1;
        console.log(count);
        if(limitimg && limitimg < count){
          tusi('上传图片不能超过'+limitimg+'张！');
          return false;
        }
        readFile(this);
      });
    ...
    function readFile(_this){  
        for(var i=0;i<_this.files.length;i++)
        {
            ...
            reader.onload = function(e){  
                $(_this).parent().siblings('.img-list').append('<img src="'+this.result+'" onclick="del_pic('+this+')" alt=""/>');                  
                var data = "";
                //如果图片大小小于500kb，则直接上传 
                if (file.size <= maxsize) {
                     data = this.result;
                    $.post(uploadUrl,{imgData:data,Orientation:Orientation,type:"payUpload"},function(data){
                      var imgurl = $(_this).parent().siblings('.imgurl').val();
                      // console.log(imgurl);
                      ...
                      $(_this).parent().siblings('.imgurl').val(imgurl+data);
                    });
                 }else{
                    var myImg = new Image(); 
