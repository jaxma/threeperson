...
var pinyin = ["hebei", "shanxi", "neimenggu", "liaoning", "jilin", "heilongjiang", "jiangsu", "zhejiang", "anhui", "fujian", "jiangxi", "shandong", "henan", "hubei", "hunan", "guangdong", "guangxi", "hainan", "sichuan", "guizhou", "yunnan", "xizang", "shanxi1", "gansu", "qinghai", "ningxia", "xinjiang", "beijing", "tianjin", "shanghai", "chongqing", "xianggang", "aomen", "taiwan"];
var color = ['#00ccff', '#2f4554', '#61a0a8', '#d48265', '#91c7ae', '#749f83', '#ca8622', '#bda29a', '#6e7074', '#546570', '#c4ccd3'];
...
//$('#totalMoney-num').text(Number($('#totalMoney').val()).formatMoney(2,'￥',',','.'))
...
require(['echarts', 'china'], function(echarts) {
  ...
  //获取省级分布
  getProvince(myChart1)

  //获取等级分布
  getlevel(myChart5);

  //获取商品类别占比
  $("#select-time").val(getNowMonth())
  var months = getNowDay();
  months = changeDate(months)
...
//    console.log(param)
    if(!isNaN(param.value)) {
      ...
      if(mapDetail.iscity != true) {
        var temp = pinYin(param.name);
        if(!reg.test(temp)){
          mapDetail.pinyin = temp;
        }else{
          alert("该地区暂无更多数据！");
          return false;
        }
...
//  mapDetail.mapdata = data1;
  ...
  //  console.log(mapDetail)
  getDataMap(mapDetail.urls, mapDetail.mapname,  echarts);
}
...
//公用的加载地图方法
function getDataMap(urls, strs, echarts) {
  //  console.log(mapdata)
  var reg = new RegExp("[\\u4E00-\\u9FFF]+","g");
  ...
  $.get(urls, function(data) {
    ...
    if(strs == 'china') {
      ...
      type = 'province';
      $.each(pinyin, function(key, value) {
        if(value == strs) {
          area = chinese[key];
        }
      });
...
//  console.log(getMaps(type, area))
    myChart4.setOption({
      series: [{
        type: 'map',
        map: strs,
        data: getMaps(type, area)
      }]
    });
  });
}

function getMaps(type, area) {
  var temp = [];
  $.ajax({
    type: "post",
    url: getAreadata,
