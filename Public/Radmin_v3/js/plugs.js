...
define(['jquery'], function () {
    ...
    msg.prototype.auto = function (data, time) {
        ...
        if (parseInt(data.status) === 1) {
            return self.success(data.info, time, function () {
                var jump_url = data.url;
                var arr_url = jump_url.split("/");
                var len = arr_url.length;
                if ((arr_url[len-1] == 'index?spm=m-0-0' &&  arr_url[len-2] == 'analysis') || (arr_url[len-1] == 'index' &&  arr_url[len-2] == 'login') || (arr_url[len-1] == 'logout' &&  arr_url[len-2] == 'index')) {
    ...
    _form.prototype.iframe = function (url, title) {
        return layer.open({title: title || '窗口', type: 2, area: ['60%', '80%'], fix: true, maxmin: false, content: url});
    };
...
//      ($.cookie('menu-style') !== 'mini') && $targetmenu.trigger('click');
        if(($.cookie('menu-style') !== 'mini')){
          $.cookie('menu-style','full');
        }else{
          $('.framework-body').addClass('framework-sidebar-mini').removeClass('framework-sidebar-full')
        }
        ...
        window.onhashchange = function () {
            ...
            if ($menu.size() > 0) {
                ...
                if($menu.hasClass('only')){
                  $menu.addClass('active');
                }else if($('.framework-body').hasClass('framework-sidebar-mini')){
                  ...
                  $li.parents('.treeview-menu').siblings('a').addClass('active');
                  $menu.parents('[data-menu-box]').removeClass('hide').siblings('[data-menu-box]').addClass('hide');
                }else{
                  var $li = $menu.parent('li').addClass('active');
                  $li.parents('.framework-sidebar').find('li.active').not($li).removeClass('active');
                  $menu.parents('.sidebar-trans').removeClass('hide').show();
                  $li.parents('.treeview-menu').addClass('open').siblings('a').addClass('active');
                  $menu.parents('[data-menu-box]').removeClass('hide').siblings('[data-menu-box]').addClass('hide');
                }
            } else {
