...
form.on('select(rb_sel)', function (data) {
    ...
    var chinese = ['一', '二', '三'];
    for (var i = 1; i <= num; i++) {
        var html = '<div class="layui-input-inline inp"><input type="text" name="param['+ level +'][]" lay-verify="required" placeholder="' + chinese[i - 1] + '级返利' + '" autocomplete="off" class="layui-input"></div>';
        temp.push(html);
    }
...
//          console.log(set_config[info][level]["param"+i]);
            var html = '<div class="layui-input-inline inp"><input type="text" name="param[' + level + '][]" value="' + set_config[info][level]["param"+i] + '" lay-verify="required" placeholder="' + chinese[i - 1] + '级返利' + '" autocomplete="off" class="layui-input"></div>';
            temp.push(html);
        }
//      console.log(temp);
...
var rb7_holder1 = "金额",
    rb7_holder2 = "比例";
function addGrad1() {
    var html = '<tr><td><input type="text" name="achievement[]" class="tb_input" placeholder="请输入' + rb7_holder1 + '"/></td><td><input type="text" name="parameter[]" class="tb_input" placeholder="请输入' + rb7_holder2 + '"/></td></tr>';
    $("#table-grad1 tbody").append(html);
}

form.on('switch(rb7_open)', function (data) {
    if (data.elem.checked) {
//      $('#rebate4').find('select').prop('disabled', false);
//      $('#rebate4').find('input').prop('disabled', false);
//      $('#rebate4').find('checkbox').prop('disabled', false);
        $("#add-grad1").bind('click', addGrad1);
    } else {
//      $('#rebate4').find('select').prop('disabled', true);
//      $('#rebate4').find('input').prop('disabled', true);
//      $('#rebate4').find('checkbox').prop('disabled', true);
...
form.on('radio(rb7_co)', function (data) {
    if (data.elem.checked) {
        $('#count-num1').attr('disabled', false);
        form.render();
    }
})
form.on('radio(rb7_cc)', function (data) {
    if (data.elem.checked) {
        $('#count-num1').attr('disabled', true);
        $('#count-money1').click();
        rb7_holder1 = "金额";
        rb7_holder2 = "比例";
        $("#table-grad1 thead tr th:eq(0)").text(rb7_holder1);
        $("#table-grad1 thead tr th:eq(1)").text(rb7_holder2 + "%");
        $.each($("#table-grad1 tbody tr").find('td:eq(0)'), function (key, value) {
            $(value).children('input').attr("placeholder", "请输入金额");
        })
        $.each($("#table-grad1 tbody tr").find('td:eq(1)'), function (key, value) {
            $(value).children('input').attr("placeholder", "请输入比例");
        })
        form.render();
    }
})
form.on('radio(rb7_cm)', function (data) {
    if (data.elem.checked) {
        rb7_holder1 = "金额";
        rb7_holder2 = "比例";
        $("#table-grad1 thead tr th:eq(0)").text(rb7_holder1);
...
form.on('radio(rb7_cn)', function (data) {
    if (data.elem.checked) {
        rb7_holder1 = "数量";
        rb7_holder2 = "金额";
        $("#table-grad1 thead tr th:eq(0)").text(rb7_holder1);
...
//初始化团队奖励
if (!$("#rebate7").find("input[lay-skin='switch']").is(':checked')) {
//    $('#rebate4').find('select').prop('disabled', true);
//    $('#rebate4').find('input').prop('disabled', true);
//    $('#rebate4').find('checkbox').prop('disabled', true);
//  $("#rebate4").find("input[lay-skin='switch']").attr('disabled', false);
...
if ($("#count-order1").is(":checked")) {
    $('#count-num1').attr('disabled', false);
}
if ($('#count-coin1').is(":checked")) {
    $('#count-num1').attr('disabled', true);
}
if ($('#count-num1').is(":checked")) {
    rb7_holder1 = "数量";
    rb7_holder2 = "金额";
    $("#table-grad1 thead tr th:eq(0)").text(rb7_holder1);
    $("#table-grad1 thead tr th:eq(1)").text(rb7_holder2);
